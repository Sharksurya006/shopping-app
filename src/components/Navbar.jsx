...
export const Logo = () => <Link to="/" className="font-display font-bold text-2xl tracking-[0.35em] pl-[0.35em]" aria-label="NAYARA home">NAYARA</Link>;
...
export default function Navbar() {
	const [open, setOpen] = useState(false); const cart = useStore(s => s.cart.reduce((a, c) => a + c.qty, 0)), wish = useStore(s => s.wishlist.length), theme = useStore(s => s.theme), toggle = useStore(s => s.toggleTheme);
	return <header className="sticky top-0 z-40 bg-bg border-b border-line">
		...
		<div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
			...
			<nav className="hidden font-bold lg:flex gap-5 text-[13px]" aria-label="Main">{NAV.map(([l, to]) => <NavLink key={l} to={to} className="hover:underline underline-offset-8">{l}</NavLink>)}</nav>
