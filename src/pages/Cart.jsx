...
export const useTotals=()=>{const cart=useStore(s=>s.cart),coupon=useStore(s=>s.coupon);const lines=cart.map(c=>({...products.find(p=>p.id===c.id),qty:c.qty}));const sub=lines.reduce((a,l)=>a+l.price*l.qty,0);
...
export default function Cart(){const {lines,sub,disc,ship,tax,total}=useTotals();const {setQty,removeFromCart,toggleWish,setCoupon}=useStore();const [code,setCode]=useState(''),[msg,setMsg]=useState('');useEffect(()=>{document.title='Your bag | NAYARA'},[]);
