...
export function Signup(){const {register,handleSubmit,watch,formState:{errors,isSubmitting}}=useForm(),nav=useNavigate(),setUser=useStore(s=>s.setUser);
...
export function Otp(){const loc=useLocation(),nav=useNavigate();const {email='',mobile='',next='/account'}=loc.state||{};
...
const verify=async()=>{const code=d.join('');if(code.length<6){setErr('Enter all 6 digits');setShake(true);setTimeout(()=>setShake(false),400);return}
...
export function Forgot(){const [s,setS]=useState(0),[v,setV]=useState('');
